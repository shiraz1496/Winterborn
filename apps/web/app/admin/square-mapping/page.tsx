...
type FieldKey = `item:${string}` | `variation:${string}`
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Link each local product to its Square catalog IDs. Paste the item ID under the item name, and the variation ID next to each colour/size. Once mapped, Square POS sales decrement stock here automatically. A blank field clears the link."
      />
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                  ...
                  <tbody>
                    {group.variations.map((v) => {
                      ...
                      return (
                        <tr key={v.variationId} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                          <td style={{ padding: '8px' }}>{v.colourFamilyName}</td>
                          <td style={{ padding: '8px' }}>{v.sizeOptionName}</td>
